...
def generate_html_table(headers, data, isRoom, isExternal=False):
    ...
    # Setting column span based on the conditions
    if isRoom:
        cols = 2
        html_table += '<table style="font-size:80%; width:100%; font-weight:400; text-align:center; border-collapse: collapse;">\n'
    else:
        html_table += '<table style="font-size:80%; width:100%; font-weight:400; text-align:center; border-collapse: collapse;">\n'
        cols = 1

    # If it's external and capacity is 30, set cols to 1
    if isExternal:
        cols = 1
    ...
    # Adding table rows and cell data
    for row in data:
        html_table += "<tr>"
        for x in row:
            for value in x:
                # Ensure hall ticket numbers are on a single line and the cells have enough space
                html_table += f'<td style="font-size:120%; width:200%; height:40px; border: 1px solid black; white-space: nowrap;">{value if value != "-1" else " "}</td>'
        html_table += "</tr>\n"
...
if(file and selected_blocks and (selected_halls or selected_rooms) and branch):
    ...
    for count, i in enumerate(res.keys()):
        ...
        with tabs[count]:
            ...
            headers = []
            if(i in selected_rooms):
                if(i in logic.vs_rooms):
                    headers = ["DESK "+ str(i+1) for i in range(4)]
                else:
                    headers = ["DESK "+ str(i+1) for i in range(5)]
            elif(i in selected_halls):
                headers = ["DESK "+ str(i+1) for i in range(6)]
